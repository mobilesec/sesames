...
public class FPIParser 
{
	...
	public FPIParser(InputStream _is) throws XmlPullParserException
	{
		mParser = Xml.newPullParser();
		mParser.setInput(new InputStreamReader(_is));
	}
	
	public FPIParser(XmlPullParser _parser)
	{
		mParser = _parser;
	}
		...
		try 
		{
			while((type=mParser.next())!=XmlPullParser.END_DOCUMENT)
			{
				if(type == XmlPullParser.START_TAG)
				{
					...
					if(tag.equals("Room"))
					{
						mRooms.add(parseRoom(mParser));
					}
					if(tag.equals("FingerPrintItem"))
					{
						mFPIs.add(parseFingerPrintItem(mParser));
					}
				}
			}
